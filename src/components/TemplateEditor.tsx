...
export default function TemplateEditor({ resumeData, setResumeData, onBack, templateId }: Props) {
  ...
  const addSkill = () => {
    const newSkill: Skill = { id: `skill${Date.now()}`, name: "", category: "Framework/Library" };
  ...
  const handleExperienceAchievementChange = (expId: string, achId: string, value: string) => {
    const updatedExperiences = resumeData.experiences.map(exp => {
      if (exp.id === expId) {
  ...
  const removeExperienceAchievement = (expId: string, achId: string) => {
    const updatedExperiences = resumeData.experiences.map(exp => {
      if (exp.id === expId) {
        return { ...exp, achievements: exp.achievements.filter(ach => ach.id !== achId) };
      }
      return exp;
    });
    setResumeData({ ...resumeData, experiences: updatedExperiences });
  ...
  const handleProjectAchievementChange = (projId: string, achId: string, value: string) => {
    const updatedProjects = resumeData.projects.map(proj => {
      if (proj.id === projId) {
  ...
  const addProjectAchievement = (projId: string) => {
    const updatedProjects = resumeData.projects.map(proj => {
      if (proj.id === projId) {
        const newAchievement: Achievement = { id: `ach${Date.now()}`, description: "" };
        return { ...proj, achievements: [...proj.achievements, newAchievement] };
      }
      return proj;
    });
    setResumeData({ ...resumeData, projects: updatedProjects });
  ...
  const removeProjectAchievement = (projId: string, achId: string) => {
    const updatedProjects = resumeData.projects.map(proj => {
      if (proj.id === projId) {
        return { ...proj, achievements: proj.achievements.filter(ach => ach.id !== achId) };
      }
      return proj;
    });
    setResumeData({ ...resumeData, projects: updatedProjects });
  ...
  const addCertification = () => {
    const newCert: Certification = { id: `cert${Date.now()}`, name: "", organization: "", date: "" };
    setResumeData({ ...resumeData, certifications: [...resumeData.certifications, newCert] });
  };
  const removeCertification = (id: string) => {
    setResumeData({ ...resumeData, certifications: resumeData.certifications.filter(cert => cert.id !== id) });
  };
  ...
  const removeLanguage = (id: string) => {
    setResumeData({ ...resumeData, languages: resumeData.languages.filter(lang => lang.id !== id) });
  };
          ...
          <AnimatePresence>
            {resumeData.skills.map(skill => (
              <motion.div 
                ...
                exit={{ opacity: 0, x: -50 }}
              >
                <Button variant="ghost" size="sm" onClick={() => removeSkill(skill.id)} className="absolute top-2 right-2 p-1">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Select onValueChange={(value) => handleSkillChange(skill.id, 'category', value)} defaultValue={skill.category}>
          ...
          <AnimatePresence>
            {resumeData.experiences.map(exp => (
              ...
              >
                ...
                <Input value={exp.title} onChange={(e) => handleExperienceChange(exp.id, "title", e.target.value)} placeholder="Title (e.g., Software Engineer)" />
                <Input value={exp.company} onChange={(e) => handleExperienceChange(exp.id, "company", e.target.value)} placeholder="Company" />
                <Input value={exp.location || ''} onChange={(e) => handleExperienceChange(exp.id, "location", e.target.value)} placeholder="Location (Optional)" />
                <div className="flex gap-2">
                  ...
                  {exp.achievements.map((ach, index) => (
                    ...
                    >
                      <Textarea 
                        ...
                        onChange={(e) => handleExperienceAchievementChange(exp.id, ach.id, e.target.value)} 
                        placeholder={`Achievement ${index + 1}`} 
                        rows={2} 
                      />
                      <Button variant="ghost" size="sm" onClick={() => removeExperienceAchievement(exp.id, ach.id)} className="p-1">
                        <Trash className="w-4 h-4" />
                      </Button>
                    </motion.div>
                  ))}
                </AnimatePresence>
                <Button size="sm" variant="outline" onClick={() => addExperienceAchievement(exp.id)} className="flex items-center gap-1">
          ...
          <AnimatePresence>
            {resumeData.projects.map(proj => (
              <motion.div 
                ...
                exit={{ opacity: 0, x: -50 }}
              >
                <Button variant="ghost" size="sm" onClick={() => removeProject(proj.id)} className="absolute top-2 right-2 p-1">
                  <Trash className="w-4 h-4" />
                </Button>
                <Input value={proj.name} onChange={(e) => handleProjectChange(proj.id, "name", e.target.value)} placeholder="Project Name" />
                <Input value={proj.techStack} onChange={(e) => handleProjectChange(proj.id, "techStack", e.target.value)} placeholder="Tech Stack (e.g., React, Node.js)" />
                <Input value={proj.githubLink || ''} onChange={(e) => handleProjectChange(proj.id, "githubLink", e.target.value)} placeholder="GitHub Link (Optional)" />
                <Input value={proj.liveLink || ''} onChange={(e) => handleProjectChange(proj.id, "liveLink", e.target.value)} placeholder="Live Demo Link (Optional)" />
                ...
                <AnimatePresence>
                  {proj.achievements.map((ach, index) => (
                    ...
                    >
                      <Textarea 
                        value={ach.description} 
                        onChange={(e) => handleProjectAchievementChange(proj.id, ach.id, e.target.value)} 
                        placeholder={`Feature ${index + 1}`} 
                        rows={2} 
                      />
                      <Button variant="ghost" size="sm" onClick={() => removeProjectAchievement(proj.id, ach.id)} className="p-1">
                        <Trash className="w-4 h-4" />
                      </Button>
                    </motion.div>
                  ))}
                </AnimatePresence>
                <Button size="sm" variant="outline" onClick={() => addProjectAchievement(proj.id)} className="flex items-center gap-1">
          ...
          <AnimatePresence>
            {resumeData.certifications.map(cert => (
              <motion.div 
                ...
                exit={{ opacity: 0, x: -50 }}
              >
                <Button variant="ghost" size="sm" onClick={() => removeCertification(cert.id)} className="absolute top-2 right-2 p-1">
                  <Trash className="w-4 h-4" />
                </Button>
                <Input value={cert.name} onChange={(e) => handleCertificationChange(cert.id, 'name', e.target.value)} placeholder="Certificate Name" />
                <Input value={cert.organization} onChange={(e) => handleCertificationChange(cert.id, 'organization', e.target.value)} placeholder="Issuing Organization" />
                <Input value={cert.date} onChange={(e) => handleCertificationChange(cert.id, 'date', e.target.value)} placeholder="Date Issued (e.g., June 2024)" />
                <Input value={cert.url || ''} onChange={(e) => handleCertificationChange(cert.id, 'url', e.target.value)} placeholder="Credential URL (Optional)" />
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
          ...
          <AnimatePresence>
            {resumeData.languages.map(lang => (
              <motion.div 
                ...
                exit={{ opacity: 0, x: -50 }}
              >
                <Button variant="ghost" size="sm" onClick={() => removeLanguage(lang.id)} className="absolute top-2 right-2 p-1">
                  <Trash className="w-4 h-4" />
                </Button>
                <div className="flex gap-2">
                  <Input 
                  ...
                  />
                  <Select onValueChange={(value) => handleLanguageChange(lang.id, 'proficiency', value)} defaultValue={lang.proficiency}>
                    <SelectTrigger className="w-[180px]">
